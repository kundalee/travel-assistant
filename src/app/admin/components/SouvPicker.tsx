...
import { fmt, includesQ } from '../utils'
import { AsyncButton, Chips, Empty, Hl, Icon, Modal, SearchBox } from '../../../components'

interface Props {
  title: string
  initial: string[] /* 已串聯的 product id */
  /** 回傳 Promise 時，等待期間按鈕顯示讀取中 */
  onApply: (chosen: Souvenir[]) => unknown
  onClose: () => void
}

/* 紀念商品挑選器：連結紀念商品資料庫（全文檢索 + 分類 + 複選） */
export default function SouvPicker({ title, initial, onApply, onClose }: Props) {
  const data = useAdminData('products').data!
  ...
  async function addProduct() {
    ...
    const saved = await create('products', {
      name, price_twd: price, bonus_twd: Math.round(price * 0.12), kind: 'souvenir', kinds: ['紀念商品'],
      cat: CATS.includes(cat) ? cat : '其它優選商品',
    }, '已新增至紀念商品資料庫', 'database-plus')
    if (!saved) return
    setSel((s) => new Set(s).add(saved.id))
    ...
    <Modal onClose={onClose} title="挑選紀念商品" sub={title || '從紀念商品資料庫選擇（可複選）'}>
      ...
      {adding && (
        <div className="filt-row">
          ...
          <AsyncButton className="btn btn-primary btn-sm" onClick={addProduct}><Icon name="plus" />新增</AsyncButton>
        </div>
      )}
      <div className="action-2">
        <button className="btn btn-ghost" onClick={() => setAdding(!adding)}><Icon name="plus" />資料庫沒有？新增商品</button>
        <AsyncButton className="btn btn-primary" onClick={apply}><Icon name="check" />加入所選（{sel.size}）</AsyncButton>
      </div>
    </Modal>
  )
}
